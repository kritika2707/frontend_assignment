...
export default function CommentsApi (props) {
    ...
    return <>
        ...
        <tbody>
        {props.comments && props.comments.map((comment)=><tr key={comment.id}><td>{comment.id}</td><td>{comment.name}</td><td>{comment.email}</td><td>{comment.body}</td><td><button className='delete-btn'><DeleteForeverIcon onClick={()=>props.del(comment.id)}/></button></td></tr>)}
        </tbody>
        </table>
    </>
}
