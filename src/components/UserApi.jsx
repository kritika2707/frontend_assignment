...
export default function UserApi (props) {
    return <>
        ...
        {props.users && props.users.map((user,index)=><tr key={index}><td>{user.id}</td><td>{user.name}</td><td>{user.username}</td><td>{user.email}</td><td>{user.address['suite']},{user.address['street']},{user.address['city']},{user.address['zipcode']}</td><td>{user.phone}</td><td>{user.website}</td><td>{user.company['name']}</td><td><button className='delete-btn'><DeleteForeverIcon onClick={()=>props.del(user.id)}/></button></td></tr>)}
        </tbody>
        </table>
    </>
}
